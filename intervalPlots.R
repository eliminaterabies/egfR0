...
gg <- (ggplot(interval_merge, aes(Days,fill=Type,color="black"))
	...
	+ geom_text(mapping=aes(x=Mean, y=0, label=paste(round(Mean,2),"days")), size=4, angle=00, vjust=-45, hjust=-0.1) 
	+ xlim(c(minDays,maxDays))
	+ ylab("Counts")
)
...
gg_gen <- (gg %+% (interval_merge %>% filter(Type == "Generation Interval")) 
	+ geom_text(data=filter(eyeball_lab_df,Type == "Generation Interval"), mapping=aes(x=xloc, y=yloc, label=paste(round(Mean,2),"days")), size=4, angle=00) 
)

gg_dogs <- (gg %+% (interval_merge %>% filter(Type == "Incubation Period: Dogs")) 
	+ geom_text(data=filter(eyeball_lab_df,Type == "Incubation Period: Dogs"), mapping=aes(x=xloc, y=yloc, label=paste(round(Mean,2),"days")), size=4, angle=00)
)
...
gg_adjbiter <- (gg %+% (interval_merge %>% filter(Type == "Weighted Incubation Period")) 
	...
	, mapping=aes(x=xloc, y=yloc, label=paste(round(Mean,2),"days")))
	)
...
print(plot_grid(gg_dogs, gg_adjbiter,ggbites, gg_gen))
