...
We first fitted logistic models, with negative binomial observation error, to incidence data to estimate \littler implemented in the R package ``epigrowthfit''. \mli{cite, waiting for MJ} 
We then compute a sample of 300\mli{nsamp from slow}$\hat{\rzero}$ values using equation (\ref{eq:RE}); for each value of $\hat{\rzero}$, we first draw a value of $\hat{\littler}$ from a normal distribution from the estimates of the logistic fit and 100 \G from the empirical contact tracing data. \mli{fix me, use R to fill in the sample numbers.}
