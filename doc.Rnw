...
%% JD: I feel like this is left over from some older, complicated discussion. We are simply observing these directly here, not reweighted.
% To account for multiple transmission, the incubation periods needs to be reweighted by the number of transmissions.
In cases where transmission links are not directly observed, it would be necessary to consider the possibility that serial-interval observations would ned to be reweighted to account for  multiple transmissions. In our case, these effects can be accounted for directly by relying on generation intervals observed through contact tracing.
...
Previous estimates of \rzero are shown in blue highlights; \rzero estimates and confidence interval (95\% quantiles from the estimated \rzero sample) from our hybrid approach using exponential (dotted lines) and logistic (solid lines). \rzero are corrected for vaccination coverage.
\flabel{Rzero}
\end{figure}
\end{center}
