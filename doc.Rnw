...
load("slow/bitten.rda")
load("slow/egf_R0.rda")
load("slow/intervals.rda")
@
...
Previous estimates of \rzero are shown in blue highlights; \rzero estimates and confidence interval (95\% quantiles from the estimated \rzero sample) from our hybrid approach using the logistic model. \rzero values are corrected for vaccination coverage.
\flabel{Rzero}
\end{figure}
\end{center}
