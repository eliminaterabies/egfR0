...
\mr{meaning of "neglected" unclear... perhaps "the work of eliminating rabies is often neglected"
I also don't understand the logic of neglect... if this speeculation belongs here it may need more detail.}
Estimates of the intrinsic reproductive number (\rzero) of canine rabies from a wide range of times and locations are low (values \textless 2), with narrow confidence intervals. The persistence of rabies across environments that vary enormously in ecological conditions is thus surprising.
\mr{as written, this assumes readers understand or at least know that R_0 has to be high to enable persistence... also doesn't flu typically have a low R_0? Maybe another sentence or half needed to explain basis for surprise.}
...
\mr{is epigrowthfit still a Bayesian method? Isn't a key reason your estimates are larger that you fit a different model than KH did? Should that be an abstract-level methods note?}
Our \rzero estimates are larger than previous estimates (on average closer to 2), with wider confidence intervals.
...
\mr{want to mention that roughly half the deaths are children?}
...
\mr{I saw something about these efforts experiencing a COVID setback, discuss?}
...
\mr{going back to earlier emails, is this honest? Isn't dog vaccination cheap enough that the strategy should be to do it to very high coverage everywhere all the time?}
...
\mr{I think the idea that different densities and population structures imply different R_0 might need to be articulated more explicitly here. Also probably don't need to use the word "surprise" in both these sentences, especially if one of them does a bnetter job setting up the expectation. }
...
\mr{why are both measles and rinderpest mentioned here with separate e.g.s?}
...
\mr{I think this shoudl go before the statements of surprise, as it sets the expectation}
...
\mr{Maybe this is the intent, but I experience some uncomfortable cognitive dissonance reading these two sentences... I think "apparent" is potentially confusing because even though R_0~1 is what has been estimated, it is apparently not right?}
...
This formula is convenient to calculate point estimates of \rzero; however, propagating uncertainty from the estimates of \littler and the \G distribution can be difficult. \mr{Do you wnat to replace "can be difficult" with something like "hasn't been done often" or "isn't part of the standard epi protocol" or something more empirical?}
...
In an earlier, influential paper, \mr{is "influential" helpful here? Useful to narrow the scope to rabies?} estimated generation intervals were constructed by summing two quantities: a latent period (the time from infection to infectiousness), and a wait time (time from infectiousness to transmission) \citep{hampson2009transmission}.
...
We then matched bootstrap samples of \G to samples from the \littler samples to produce a range of estimates for \rzero.
This hybrid bootstrap approach incorporates both sources of uncertainties from \littler and \G (frequentist empirical bootstrap) when calculating \rzero estimates.
