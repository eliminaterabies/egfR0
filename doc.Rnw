...
In their analysis, latent (really, incubation) periods and infectious periods were randomly and independently resampled from empirically observed distributions \citep{hampson2009transmission}, and then waiting times sampled uniformly from the selected infection periods.
